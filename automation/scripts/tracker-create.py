...
PRESETS: dict[str, dict] = {
    ...
    # Трудозатраты без артефакта: встречи, созвоны, переписка — работа, которая
    # съедает часы, но ничего не меняет в продукте и потому не имеет дома ни
    # в одной другой очереди. Пилот на одном клиенте; кто заводит встречу
    # и какие поля нужны — решается по итогам пилота, не заранее.
    #
    # Разговор ПРО ЗАДАЧУ сюда не идёт: для него есть тип «Консультация»
    # в CRM (пресет crm-konsultaciya). Здесь — разговор про клиента,
    # договор, объём и процесс.
    #
    # Участники пока живут в теле задачи: отдельного поля в очереди нет.
    "effort-vstrecha": {
        "queue": "EFFORT",
        ...
        "prefix": None,
        "client_required": True,
        "client_prefix": True,          # «PFZ Статус по интеграции»
    },
}
...
MNEMONICS: dict[str, str] = {
    ...
    # Пилот, НЕ в регламенте. Pfizer нашим клиентом ещё не является: мнемоника
    # назначена, чтобы часы пилота в EFFORT было чем тегировать, но в таблицу
    # мнемоник «Описания очередей» она не внесена и вносится только вместе
    # с договором. Бэклог: Пф-13.
    "PFZ": "Pfizer (пилот, не в регламенте)",
}
