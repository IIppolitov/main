...
[claude-code/osnovy.md](claude-code/osnovy.md) — сама страница раздела, под ней
[разработка](claude-code/razrabotka.md) и [личный проект](claude-code/lichnyj-proekt.md),
а три документа команд — дочерние уже к разработке. Указатель [claude-code/README.md](claude-code/README.md) остаётся только здесь: в Вики его
...
**Локальный стенд админки** — [pbeadmin-local.md](pbeadmin-local.md), инструкция
тестировщикам: как поставить окружение на macOS (PHP, драйверы MSSQL), настроить
SSH-ключ, развернуть `pbeadmin` у себя, заполнить `.env`, подключить модули и
переключаться между версиями. Второе исключение из правила «в регламентах
не упоминаем скрипты» — как и [claude-code/wiki.md](claude-code/wiki.md): скрипты
`deploy.sh` и `enable-module.sh` здесь и есть содержание, и лежат они не в нашем
рабочем пространстве, а в репозитории `deploy-scripts`, который читатель получает
вместе с инструкцией. Ссылок в наши документы и в бэклог в тексте нет.

Первый раздел документа — запрет на тестирование в боевых базах и порядок получения
реквизитов к тестовым. Он там первый намеренно: остальное — техника, а это правило.
...
Полный разбор работы с Вики — доступ, API, публикация обратно, накопленные
грабли и режим для тех, у кого нет терминала — в
[claude-code/wiki.md](claude-code/wiki.md).
