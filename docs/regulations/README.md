...
тестировщикам: как настроить SSH-ключ, развернуть `pbeadmin` у себя, заполнить
`.env`, подключить модули и переключаться между версиями. Второе исключение из правила «в регламентах
